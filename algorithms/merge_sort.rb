...
def merge_sort(items)
  ...
  # Split and recurse
  midpoint = (items.length/2).to_i
  left = items[0...midpoint]  # For 4 items, left is 0, 1
  right = items[(midpoint)...items.length]  # right is 2, 3

  left = merge_sort(left)
  right = merge_sort(right)

  # Merge resulting lists, assuming they are now sorted
  result = []
...
class MergeSortTest < Minitest::Test
  def test_can_sort_list
    result = merge_sort([4, 3, 2, 5, 1])
    expected = [1, 2, 3, 4, 5]
    assert_equal expected, result
  end

  def test_can_sort_list_with_doubles
    result = merge_sort([4, 3, 2, 5, 5, 5, 1])
  ...
  def test_can_sort_a_lot_of_items
    result = merge_sort([53,88,12,87,39,58,55,10,6,89,46,79,38,49,42,45,2,56,81,21,25,92,28,85,59,22,66,17,33,4,8,23,36,71,93,43,13,61,69,78,14,98,65,68,27,30,64,63,67,82,40,74,86,3,20,7,5,83,35,60,15,51,76,19,72,48,91,18,84,80,24,62,77,9,11,52,75,41,96,37,47,70,44,50,99,57,97,34,90,1,95,29,73,100,31,54,26,32,16,94])
